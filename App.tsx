...
const App: React.FC = () => {
  ...
  const handleDeleteTransaction = (id: string) => {
    setConfirmModal({
      ...
      message: 'El saldo de tus cuentas se revertirá automáticamente al borrar este movimiento.',
      onConfirm: async () => {
  ...
  const handleDeleteAccount = (id: string) => {
    setConfirmModal({
      ...
      message: 'Se perderán todos los movimientos y saldos de esta cuenta.',
      onConfirm: async () => {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center font-black text-slate-400 italic italic tracking-widest uppercase">Finanza360...</div>;
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-slate-50 text-slate-900 font-sans overflow-x-hidden">
      ...
      <aside className={`fixed inset-0 z-40 md:relative md:translate-x-0 md:w-80 bg-white border-r p-8 flex flex-col transition-transform ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex items-center space-x-3 mb-10">
          ...
          <span className="text-2xl font-black tracking-tighter uppercase">Finanza360</span>
        </div>
